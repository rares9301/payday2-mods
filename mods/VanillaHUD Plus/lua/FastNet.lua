    ...
    function MenuCallbackHandler:_find_online_games(friends_only)
		if self:is_win32() then
			local function f(info)
				...
				managers.menu:active_menu().logic:refresh_node(VHUDPlus.fast_net, true, info, friends_only)
			end
			...
			local usrs_f = function(success, amount)
				...
				if success then
					...
					local is_FastNet = (managers.menu:active_menu().logic:selected_node():parameters().name == VHUDPlus.fast_net)
		...
		if self:is_ps3() or self:is_ps4() then
			...
			local function f(info_list)
				...
				managers.network.matchmake:search_lobby_done()
				managers.menu:active_menu().logic:refresh_node(VHUDPlus.fast_net, true, info_list, friends_only)
			end
			managers.network.matchmake:register_callback("search_lobby", f)

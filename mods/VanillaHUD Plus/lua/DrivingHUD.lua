if string.lower(RequiredScript) == "lib/managers/hud/huddriving" then
	HUDDriving.VEHICLES = {
		["vhl_longfellow_name"] = {
			...
			texture = "assets/guis/textures/drivinghud"
		},
		["vhl_falcogini_name"] = {
	...
	function HUDDriving:init(hud)
		...
		self._legend_loot 		= HUDDriving.LegendImageItem:new(self._legend:panel(), self._legend, "legend_loot", HUDDriving._WIDTH / 2, HUDDriving._FONT_SIZE, { texture = "assets/guis/textures/drivinghud", text_rect = {1840, 63, 160, 137}, show_max = true })
		self._legend_passengers = HUDDriving.LegendImageItem:new(self._legend:panel(), self._legend, "legend_passengers", HUDDriving._WIDTH / 2, HUDDriving._FONT_SIZE, { texture = "assets/guis/textures/drivinghud", text_rect = {1920, 256, 128, 130}, show_max = true })
...
--		local settings = WolfHUD:getSetting({"DrivingHUD"}, {})
		local settings = VHUDPlus:getSetting({"DrivingHUD"}, {})     --DrivingHUD.settings
	...
	function HUDDriving.VehicleImageItem:init(base_panel, owner, name, width, height, params)
		...
		self._passengers = {}
		self._texture = params.texture or "assets/guis/textures/drivinghud"
...
elseif string.lower(RequiredScript) == "lib/states/ingamedriving" then
	...
	function IngameDriving:_update_driving_hud()
		if managers.player and managers.player:get_vehicle() and managers.player:get_vehicle().vehicle_unit and managers.player:get_vehicle().vehicle_unit:vehicle() then
			...
			if VHUDPlus:getSetting({"DrivingHUD", "SPEED_IN_MPH"}, false) then
				speed = speed / 1.60934 --in km/h by default apparently
			end
